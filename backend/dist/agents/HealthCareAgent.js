...
dotenv.config();
export async function handleHealthcareQuery(userMessage, sessionId) {
    ...
    const chatHistory = previousMessages.chat_history;
    const formattedHistory = chatHistory.map(msg => msg._getType() === "human" ? `User: ${msg.content}` : `Assistant: ${msg.content}`).join("\n");
    // 🛠️ Use tools
...
`.trim();
    ...
    try {
        ...
        await memory.saveContext({ input: userMessage }, { output: answer });
        return answer;
    }
    catch (err) {
